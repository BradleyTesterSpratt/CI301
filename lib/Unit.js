...
function Unit(state, position, map) {
  ...
  map.addTerrain(new Terrain(this.position));
  this.map = map;
  this.speed = 2;
  this.actionQueue = [];
  this.destination;
...
Unit.prototype.move = function() {
  ...
  for (i=0; movementAllowence > i; i++) {
    ...
    if(this.debug) console.log(terrain);
    if(terrain == null || terrain.traversable) {
      if(this.debug) console.log(terrain);
      this.position = newPosition;
      if(terrain == null) {
        this.map.addTerrain(new Terrain(this.position));
      } else if(terrain.traversable) {
        terrain.toggleTraversable();
      }
    }
  }
